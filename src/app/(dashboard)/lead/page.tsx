...
// --- LOGIKA UTAMA DRAG & DROP ---
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      alert("Failed to update data, reverting view...");
  ...
  return (
    ...
    <div className="h-full p-2 flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 gap-2"> 
        <div>
          <h2 className="text-lg sm:text-xl font-semibold text-[#2E2E2E] dark:text-white">Leads</h2>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 bg-[#5A4FB5] hover:bg-[#4a4194] text-white px-3 sm:px-5 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all shadow-sm hover:shadow-md"
          >
            <Plus size={14} className="sm:w-4 sm:h-4" /> <span className="hidden xs:inline">Add</span> Lead
          </button>

          {/* TOMBOL 2: AI CHAT (SECONDARY / DEV) - Hidden on mobile */}
          <button 
            className="hidden sm:flex items-center gap-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 px-5 py-1.5 rounded-full text-sm font-medium transition-colors opacity-60 cursor-not-allowed"
          >
            <Bot size={16} /> AI Chat
          </button>
        </div>
      ...
      {/* === TOOLBAR === */}
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4 mt-1">
        ...
        <Link 
          href="/lead/archive"
          className="flex items-center gap-1 border border-gray-300 dark:border-gray-700  px-3 py-1.5 rounded-full text-sm bg-white dark:bg-gray-800  hover:bg-gray-50 dark:hover:bg-gray-700  text-black dark:text-white transition-colors"
        >
        ...
        <div className="relative" ref={filterRef}>
             ...
             {isFilterOpen && (
               <div className="fixed sm:absolute inset-x-2 sm:inset-x-auto sm:left-0 top-auto mt-2 sm:w-80 bg-white dark:bg-[#2C2C2C] rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-4 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-left max-h-[70vh] overflow-y-auto">
...
              title={stage.title}
      ...
      {/* === FOOTER ACTION BAR === */}
      <div className="sticky bottom-0 z-20 w-full bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 px-2 sm:px-6 py-2 sm:py-4 flex justify-center gap-2 sm:gap-4 md:gap-10 rounded-t-2xl shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
            ...
            <Droppable droppableId="delete-zone">
              {(provided, snapshot) => (
                <div
                  ...
                  className={`
                    flex-1 sm:flex-none sm:w-48 md:w-72 h-10 sm:h-12 border-2 border-dashed rounded-xl font-bold text-xs sm:text-lg uppercase tracking-wider flex items-center justify-center transition-all
        ...
        // === MODE LIST ===
        <div className="h-full overflow-y-auto pt-2 no-scrollbar">
            <LeadListView 
              leads={localLeads ? Object.values(localLeads).flat() as any[] : []} 
              onDeleteLead={(leadId) => {
